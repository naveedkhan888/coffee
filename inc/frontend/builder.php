<?php
/** header desktop **/
if ( ! function_exists( 'coffeee_header_builder' ) ) {
    function coffeee_header_builder (){
        $header_builder = '';    
        ...
        if ( is_page() ) {
            if ( function_exists('rwmb_meta') ) {
                ...
                if ($metabox_fb != '') {
                    ...
                    $header_builder = coffeee_get_option('header_layout');
                }
            } 
        }else{
            $header_builder = coffeee_get_option('header_layout');
...
/** header mobile **/
if ( ! function_exists( 'coffeee_mobile_builder' ) ) {
    function coffeee_mobile_builder (){
        ...
        if ( is_page() ) {
            if ( function_exists('rwmb_meta') ) {
                ...
                if ($metabox_hmb != '') {
                    ...
                    $mobile_builder = coffeee_get_option('header_mobile');
                }
            } 
        }else{
            $mobile_builder = coffeee_get_option('header_mobile');
...
/** side panel **/
if ( ! function_exists( 'coffeee_sidepanel_builder' ) ) {
    function coffeee_sidepanel_builder (){

        $panel_builder = coffeee_get_option('sidepanel_layout');
...
/** 404 template **/
if ( ! function_exists( 'coffeee_404_builder' ) ) {
    function coffeee_404_builder (){

        $error_builder = coffeee_get_option('page_404');
        ...
        if( !$error_builder ) { ?>
            <div class="error-404 not-found text-center">
                <div class="container">
                    ...
                    <h1><?php wp_kses( _e( '404', 'coffeee' ), wp_kses_allowed_html('post')  ); ?></h1>
                    <h2><?php esc_html_e( 'Sorry! Page Not Found!', 'coffeee' ); ?></h2>
                    <div class="page-content">
                        <p><?php esc_html_e( 'Oops! The page you are looking for does not exist. Please return to the site’s homepage.', 'coffeee' ); ?></p>
                        <?php get_search_form(); ?>
                        <a class="xptf-btn" href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php esc_html_e( 'take me home', 'coffeee' ); ?></a>
                    </div>
                </div>
            </div>
        <?php }else{
...
/** footer **/
if ( ! function_exists( 'coffeee_footer_builder' ) ) {
    function coffeee_footer_builder (){
        $footer_builder = '';    
        ...
        if ( is_page() ) {
            if ( function_exists('rwmb_meta') ) {
                ...
                if ($metabox_fb != '') {
                    ...
                    $footer_builder = coffeee_get_option('footer_layout');
                }
            } 
        }else{
            $footer_builder = coffeee_get_option('footer_layout');
