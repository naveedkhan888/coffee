<?php
if ( ! function_exists( 'coffeee_page_header' ) ) {
    function coffeee_page_header (){
        $pheader = '';
        ...
        if( !coffeee_get_option('pheader_switch') && !$pheader ) {
            return;
        }else{
            $bg     = '';
            $title  = '';
            ...
            if ( is_home() ) {
                ...
                $title = esc_html__('Search Results for: ', 'coffeee') . get_search_query();
            } elseif ( is_archive() ) {
                $title = get_the_archive_title();
            } elseif ( is_singular('post') ) {
                $title = coffeee_get_option( 'ptitle_post' ) ? coffeee_get_option( 'ptitle_post' ) : get_the_title();
            }else {
                $title = get_the_title();
            }
            
            if (!function_exists('rwmb_meta')) {
                $bg = coffeee_get_option( 'pheader_img' );
            } else {
                if( is_home() ) {
        ...
        ?>        
            <div class="page-header flex-middle" <?php if ($bg) { ?> style="background-image: url(<?php echo esc_url($bg); ?>);" <?php } ?>>
                <div class="container">
                    <div class="inner <?php if( !coffeee_get_option( 'left_bread' ) ) echo 'flex-middle'; ?>">
                        <?php if( class_exists( 'woocommerce' ) && is_woocommerce() ) { ?>
                            <?php if( !is_product() ){ ?>
                                ...
                                <?php endif; ?>                            
                            <?php }else{ ?>
                                <h2 class="page-title"><?php echo esc_html( coffeee_get_option( 'page_title_product' ) ); ?></h2>
                            <?php } ?>    
                            <?php do_action( 'coffeee_woocommerce_breadcrumb' ); ?>
                        <?php }else{ ?>
                            <h1 class="page-title"><?php echo implode('', $output); ?></h1>
                        <?php 
                            if (function_exists('coffeee_breadcrumbs') && coffeee_get_option('breadcrumbs')):
                                echo coffeee_breadcrumbs();
                            endif;
                        } ?>
                    </div>
                </div>
