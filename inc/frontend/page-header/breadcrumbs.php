<?php
// Breadcrumbs
if ( ! function_exists( 'coffeee_get_breadcrumbs' ) ) {
    function coffeee_get_breadcrumbs ( 
        $list_style = 'ul', 
        $list_id = 'breadcrumbs', 
        ...
        // Front page
        if ( is_front_page() ) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__('Home', 'coffeee') . '</li>'; //get_bloginfo( 'name' )
        } else {
            $breadcrumb .= '<li><a href="' . home_url() . '">' . esc_html__('Home', 'coffeee') . '</a></li>'; //get_bloginfo( 'name' )
        }
        // Blog archive
        ...
        // Search
        if ( is_search() ) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__( 'Search', 'coffeee' ) . '</li>';
        }
        // 404
        if ( is_404() ) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__( '404', 'coffeee' ) . '</li>';
        }
        ...
        if ( is_tax() ) {
            ...
            foreach ( $attached_to as $post_type ) {
                ...
                if ( ! $output && get_post_type_archive_link( $cpt_obj->name ) ) {
                    $archive_link = '';
                    if ( 'xp_portfolio' == get_post_type() && coffeee_get_option('portfolio_archive') == 'archive_custom' ) {
                        $archive_link = get_page_link( coffeee_get_option('archive_page_custom') );                                
                    } else {
...
if ( ! function_exists( 'coffeee_breadcrumbs' ) ) {
    function coffeee_breadcrumbs (){

        if( coffeee_get_option('breadcrumbs') && !is_front_page() ){ echo coffeee_get_breadcrumbs(); } ?>
