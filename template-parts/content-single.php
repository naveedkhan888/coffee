...
<article id="post-<?php the_ID(); ?>" <?php post_class('blog-post post-box'); ?>>
    ...
    <div class="inner-post <?php if( !has_post_thumbnail() ) echo 'no-ptop'; ?>">
        <?php if( $format != 'gallery' && $format != 'image' && $format != 'video' && !has_post_thumbnail() ) coffeee_posted_in(); ?>
        <div class="entry-header">
            <div class="entry-meta">
                <?php if( coffeee_get_option( 'post_entry_meta' ) ) { coffeee_post_meta(); } ?>
            </div>
            <?php if( coffeee_get_option( 'ptitle_post' ) ) the_title( '<h4 class="entry-title">', '</h4>' ); ?>
            ...
            <?php
                ...
                the_content(sprintf(
                    ...
                    /* translators: %s: Name of current post. Only visible to screen readers */
                        __('Continue reading<span class="screen-reader-text"> "%s"</span>', 'coffeee'),
                        array(
                            'span' => array(
                                'class' => array(),
                            ),
                ...
                wp_link_pages(array(
                    'before' => '<div class="page-links">' . esc_html__('Pages:', 'coffeee'),
                    'after' => '</div>',
                ));
            ?>
        ...
        <div class="entry-footer clearfix">
            <?php coffeee_entry_footer(); ?>
        </div>
        <?php if( coffeee_get_option('author_box') ) coffeee_author_info_box(); ?>
        <?php if( coffeee_get_option('post_nav') ) coffeee_single_post_nav(); ?>
        <?php if( coffeee_get_option('related_post') ) coffeee_related_posts(); ?>
    </div>
